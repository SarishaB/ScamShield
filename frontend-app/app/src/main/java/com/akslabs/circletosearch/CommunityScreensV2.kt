            ...
            Card(colors = CardDefaults.cardColors(containerColor = ShieldSurface), shape = RoundedCornerShape(22.dp), border = BorderStroke(1.dp, ShieldViolet.copy(alpha = 0.16f))) {
                Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(11.dp)) {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Spacer(Modifier.width(10.dp)); Text("WHAT IS CHAKSHU?", style = MaterialTheme.typography.titleSmall, fontWeight = FontWeight.Bold, color = ShieldText, letterSpacing = 0.8.sp)
                    }
...
@Composable
private fun CommunityReportV2(report: CommunityReportsApi.CommunityPost) {
    val accent = when (report.category.uppercase()) {
        "PHISHING", "CREDENTIAL THEFT" -> ShieldAmber
        "PAYMENT FRAUD", "FRAUD" -> ShieldRed
        else -> ShieldViolet
    }
    Card(colors = CardDefaults.cardColors(containerColor = ShieldSurfaceRaised), shape = RoundedCornerShape(18.dp)) {
        Row(Modifier.fillMaxWidth().padding(15.dp), verticalAlignment = Alignment.Top) {
            ...
            Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(3.dp)) {
                Text(report.indicator, fontWeight = FontWeight.SemiBold, color = ShieldText)
                Text("${report.category} • ${report.type}", style = MaterialTheme.typography.bodySmall, color = ShieldMuted)
                report.description?.takeIf { it.isNotBlank() }?.let { Text(it, style = MaterialTheme.typography.bodySmall, color = ShieldText) }
                Text(formatReportTime(report.createdAt), style = MaterialTheme.typography.labelSmall, color = ShieldMuted)
            }
        }
    }
}
...
fun ReportScamScreenV2(onBack: () -> Unit) {
    ...
    Scaffold(containerColor = ShieldBackground, topBar = {
        ...
        Column(Modifier.fillMaxSize().padding(padding).padding(horizontal = 20.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            message?.let { Text(it, color = ShieldVioletBright) }; error?.let { Text(it, color = ShieldRed) }; Spacer(Modifier.height(10.dp))
        }
    }
}

@Composable
private fun ReportField(label: String, value: String, onValueChange: (String) -> Unit) { OutlinedTextField(value = value, onValueChange = onValueChange, label = { Text(label) }, modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(15.dp)) }
